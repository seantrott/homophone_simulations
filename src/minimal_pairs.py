...
def main(language, N, matched, mp_dir, phon_column="PhonDISC", nphones=5):
	"""Main script."""

	## Load files
	dir_path = "data/processed/{lan}".format(lan=language)
	art_string = "{lan}_artificial_{N}_matched_on_{match}_no_restriction_{n}phone.csv".format(lan=language, N=N, match=matched, n=nphones)
...
if __name__ == "__main__":
	...
	# Run main script
	main(language=language, N=N, matched=matched, mp_dir=mp_dir, phon_column=phon_column, nphones=nphones)
